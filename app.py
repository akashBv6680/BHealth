# app.py
# HealthAI Suite - Intelligent Analytics for Patient Care (updated HF text classification + sentiment)
...
### 🧠 RAG Chatbot Module
This is a specialized Health Consultant AI. It uses Retrieval-Augmented Generation (RAG), meaning it first consults its fixed internal Knowledge Base (KB) and, if the answer is outside the KB, it automatically uses the Google Search tool to find current information and provides citations for reliability.
...
Diabetes (Type 2) Management: Chronic high blood sugar. Managed by diet, exercise (150 mins/week), Metformin, and monitoring.
Hypertension (High Blood Pressure): High force against artery walls. Often silent. Treatment: DASH diet, exercise, ACE inhibitors, diuretics.
Asthma Control: Narrowing and swelling of airways. Symptoms: wheezing, shortness of breath. Treatment: Reliever and Preventer inhalers.
Mental Health: Depression and Anxiety. Treated with psychotherapy (CBT) and medication.
"""
...
def load_tabular_models():
    clf = Pipeline(
        [("scaler", StandardScaler()), ("rf", RandomForestClassifier(n_estimators=50, random_state=42))]
    )
    reg = Pipeline(
        [("scaler", StandardScaler()), ("rf", RandomForestRegressor(n_estimators=50, random_state=42))]
    )
    return clf, reg
...
def get_collection():
    if "db_client" not in st.session_state:
        (
            ...
            st.session_state.google_search_tool,
...
def text_classify(text: str, classifier):
    if classifier is None:
        return {"label": "unknown", "score": 0.0, "all_scores": []}
    try:
        preds = classifier(text)
        scores = preds[0] if isinstance(preds, list) and len(preds) > 0 else preds
        ...
        return {"label": "error", "score": 0.0, "all_scores": []}
...
def sentiment_text(text: str, classifier):
    if classifier is None:
...
if menu == "🧑‍⚕️ Risk Stratification":
    ...
    if submitted:
        ...
        st.session_state.module_interaction_log[menu] = {
            ...
            "result": f"Risk: {label} (Score {score}).",
        }
    ...
    if submitted:
        ...
        st.session_state.module_interaction_log[menu] = {
            "timestamp": current_time,
            "result": f"LOS {int(round(los_est))} days.",
        }
    ...
    if submitted:
        ...
        fig.update_traces(
            marker=dict(size=[10 if t == "New Patient" else 5 for t in df["Type"]]),
    ...
    if up:
        ...
        def dummy(img):
            import numpy as np
            return {
                "diagnosis": np.random.choice(["No Anomaly", "Pneumonia", "Fracture", "Mass"]),
    ...
    if st.button("Analyze Notes"):
        if not notes.strip():
            ...
            res = text_classify(notes, text_classifier)
            if res["label"] in ("error", "unknown"):
                st.error("Model not loaded.")
                desc = "Analysis failed."
            else:
                st.success(f"Primary tone: {res['label']} ({res['score']:.2f})")
                if res.get("all_scores"):
                    df_scores = pd.DataFrame(res["all_scores"]).sort_values("score", ascending=False)
    ...
    c1, c2 = st.columns(2)
    with c1:
        src = st.selectbox("Source", list(LANGUAGE_DICT.keys()), index=0)
    with c2:
        tgt = st.selectbox("Target", list(LANGUAGE_DICT.keys()), index=1)
    ...
    if st.button("Analyze Sentiment"):
        if not fb.strip():
            ...
            s = sentiment_text(fb, sentiment_classifier)
            if s["label"] in ("unknown", "error"):
                st.error("Model not loaded.")
            else:
    ...
    if "messages_assistant" not in st.session_state:
        st.session_state["messages_assistant"] = [
            {
                "role": "assistant",
                "content": "Hello! I am a general health assistant powered by Google Gemini. How can I help you today?",
            }
        ]
    ...
    if "messages_rag" not in st.session_state:
        st.session_state["messages_rag"] = [
            {
                "role": "assistant",
                "content": "Hello! I'm your context-aware RAG medical assistant. Ask any health question.",
            }
        ]

...
def synthesize(text: str, engine: str, lang_code="en"):
    ...
    if engine == "Edge-TTS":
        audio, err = tts_edge(text, voice="en-US-AriaNeural", rate="+0%")
        return (audio, "audio/mp3", err)
    if engine == "gTTS":
...
st.sidebar.caption("Voice mode renders audio with download.")
            ...
            with st.spinner("Thinking..."):
                ...
                if resp_mode == "Voice":
                    ...
                    if audio:
                        ...
                        st.download_button("Download speech", data=audio, file_name="answer.mp3", mime=mime)
                    else:
